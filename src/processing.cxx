...
void processing::SetDistanceMap( std::string distanceMap )
{
  DistanceMapName = distanceMap ;
}
void processing::SetDistanceMapFlag( int distanceMapFlag )
{
  FlagDistanceMap = distanceMapFlag ;
}
void processing::SetMaxDistance( float maxDistance )
{
...
vtkSmartPointer< vtkPolyData > processing::DistanceMap( vtkSmartPointer< vtkPolyData > polyData )
{
  vtkSmartPointer< vtkPolyData > newPolyData = vtkSmartPointer< vtkPolyData >::New();
  vtkSmartPointer<vtkFloatArray> NewTensors=vtkSmartPointer<vtkFloatArray>::New() ;
  vtkSmartPointer<vtkPoints> NewPoints=vtkSmartPointer<vtkPoints>::New() ;
  ...
  typedef itk::ImageFileReader< ImageType > ReaderType ;
  ReaderType::Pointer readerMask = ReaderType::New() ;
  readerMask->SetFileName( DistanceMapName ) ;
  readerMask->Update() ;
  ImageType::Pointer maskImage = ImageType::New() ;
  maskImage = readerMask->GetOutput() ;
  ...
  for( int i = 0 ; Lines->GetNextCell( NumberOfPoints , Ids ) ; i++ )
  {
    ...
    if( remove == false )
    {
      for( int k = 0 ; k < NumberOfPoints ; k++ )
      {
        ...
        NewId++ ;
        NewTensors->InsertNextTuple( tensors->GetTuple9( GetPointId( i , k , polyData ) ) ) ;
      }
      NewLines->InsertNextCell( NewLine ) ;
    }
    else{
...
int processing::run()
{
  ...
  if( FlagDistanceMap == true )
  {
    cleanedFiberPolyData = DistanceMap( cleanedFiberPolyData ) ;
  }
